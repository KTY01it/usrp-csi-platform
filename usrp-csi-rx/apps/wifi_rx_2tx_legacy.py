...
import ieee802_11
from csi_ltf_estimator2tx import csi_ltf_estimator
...
class mac_freq_tap(gr.basic_block):
    ...
    def _handler(self, msg):
        try:
            ...
            # Parse key=val theo cặp, không phụ thuộc thứ tự
            pairs = dict(re.findall(rb'([A-Z]+)=(\d+)', ba))
            if b'FREQ' in pairs and b'SEQ' in pairs and b'TXID' in pairs and b'TS' in pairs:
                freq_mhz = int(pairs[b'FREQ'])
                seq      = int(pairs[b'SEQ'])
                txid     = int(pairs[b'TXID'])
                ts       = int(pairs[b'TS'])
                txant    = int(pairs[b'TXANT']) if b'TXANT' in pairs else None
                # callback: truyền luôn txant (có thể None)
                self.cb(freq_mhz, seq, txid, txant, ts)
        except Exception:
            pass

class pdu_capture(gr.basic_block):
    def __init__(self, cb):
        gr.basic_block.__init__(self, name="pdu_capture", in_sig=None, out_sig=None)
        self.cb = cb
        self.message_port_register_in(pmt.intern("in"))
        self.set_msg_handler(pmt.intern("in"), self._handler)
    def _handler(self, msg):
...
class TxPairLogger:
    C = 299792458.0
    def __init__(self, tx_d_m=0.10, pair_window_ms=200.0):
        self.tx_d = float(tx_d_m)
        self.pair_win = float(pair_window_ms)
        self.freq_hz = None
        self.fh = None
        # GHÉP THEO COINC_ID, không dùng txant nữa
        # coinc_id -> {0:(meta,H), 1:(meta,H)}
        self._rxpair = {}
        # Lưu "burst hoàn chỉnh (2 RX)" trước đó, để ghép với burst hiện tại
        self._prev_pair = None
        self._writes = 0  # đếm số record đã ghi (diagnostic)

    def set_output(self, path, freq_hz, tx_d_m=None):
        if tx_d_m is not None:
            self.tx_d = float(tx_d_m)
        self.close()
    ...
    def close(self):
        try:
            if self.fh:
                print(f"[AODLOG] closed (writes={self._writes})")
                self.fh.close()
        except Exception:
            pass
        self.fh = None
    ...
    def _get(meta, key, default=None):
        ...
        if v is pmt.PMT_NIL:
            return default
        try:
            if pmt.is_integer(v): return int(pmt.to_long(v))
            if pmt.is_real(v):    return float(pmt.to_double(v))
            if pmt.is_symbol(v):  return pmt.symbol_to_string(v)
        except Exception: pass
        return default

    def _write(self, rec:dict):
        if self.fh is None: 
            # Fallback: nếu vì lý do nào đó chưa mở file, tự mở tạm ở /tmp
            try:
                os.makedirs("/tmp/aod", exist_ok=True)
                if self.freq_hz is None: self.freq_hz = 5.18e9
                auto = f"/tmp/aod/aod_pairs_{int(self.freq_hz/1e6)}MHz.jsonl"
                self.fh = open(auto, "a", buffering=1, encoding="utf-8")
                print(f"[AODLOG] (auto) → {auto}")
            except Exception:
                return
        try:
            import json
            ...
            self._writes += 1
            if (self._writes % 20) == 1:
                print(f"[AOD] wrote #{self._writes}: freq={rec.get('freq_hz')} aod_deg={rec.get('aod_deg'):.2f} dt={rec.get('pair_dt_ms')}ms")
        except Exception:
            pass

    def ingest(self, meta, data_vec):
        rxch  = self._get(meta, "rx_chan")
        coinc = self._get(meta, "coinc_id")
        freq  = self._get(meta, "freq")
        t_ms  = self._get(meta, "ts_wall_ms", int(time.time()*1000))
        if None in (rxch, coinc):
            return

        H = np.array(pmt.c32vector_elements(data_vec), dtype=np.complex64)

        key = int(coinc)
        slot = self._rxpair.setdefault(key, {})
        # LƯU CẢ meta VÀ H vào slot
        slot[int(rxch)] = (meta, H)
        ...
        if 0 in slot and 1 in slot:
            ...
            m1, H1 = slot[1]
            try:
                del self._rxpair[key]
            except Exception:
                pass

            # >>> ĐÍNH KÈM meta_rx0/meta_rx1 ĐỂ _commit_pair() DÙNG <<<
            pair = {
                "freq": int(freq if freq is not None else (self.freq_hz or 0)),
                "ts_ms": int(t_ms),
                "coinc_id": int(coinc),
                "H_rx0": H0, "H_rx1": H1,
                "meta_rx0": m0, "meta_rx1": m1,   # <— thêm 2 dòng này
            }
            self._commit_pair(pair)

    def _commit_pair(self, cur):
        prev = self._prev_pair
        self._prev_pair = cur  # LƯU LẠI để ghép với lần tới

        # chưa có "pair trước" để ghép thì thôi
        if prev is None:
            return
        # phải cùng tần số
        if int(prev["freq"]) != int(cur["freq"]):
            return
        # khoảng cách thời gian giữa 2 burst phải nhỏ
        if abs(cur["ts_ms"] - prev["ts_ms"]) > self.pair_win:
            return

        # Tính AoD
        eps = 1e-12
        R0 = (cur["H_rx0"] + eps) / (prev["H_rx0"] + eps)
        R1 = (cur["H_rx1"] + eps) / (prev["H_rx1"] + eps)
        phi_k = np.angle(R1 * np.conj(R0))
        w = (np.abs(prev["H_rx0"])*np.abs(prev["H_rx1"])*np.abs(cur["H_rx0"])*np.abs(cur["H_rx1"])) + eps
        phi_mean = np.angle(np.sum(w * np.exp(1j*phi_k)))

        lam = self.C / float(cur["freq"] if cur["freq"] > 0 else (self.freq_hz or 1.0))
        x = (phi_mean / (2*np.pi)) * (lam / max(self.tx_d, eps))
        aod_deg = float('nan')
        if -1.0 <= x <= 1.0:
            aod_deg = degrees(asin(x))

        # Lấy MAC mapping từ meta của burst trước & hiện tại (đÃ CÓ trong pair)
        prev_mac = self._pick_mac(prev["meta_rx0"], prev["meta_rx1"])
        cur_mac  = self._pick_mac(cur["meta_rx0"],  cur["meta_rx1"])

        rec = {
            "time_ms": int(time.time()*1000),
            "freq_hz": int(cur["freq"]),
            "pair_dt_ms": abs(int(cur["ts_ms"] - prev["ts_ms"])),
            "coinc_prev": int(prev["coinc_id"]),
            "coinc_cur": int(cur["coinc_id"]),
            "phi_mean_rad": float(phi_mean),
            "aod_deg": float(aod_deg),
            "tx_ant_spacing_m": self.tx_d,

            # Giữ trường cũ (chọn 1)
            "prev_mac": prev_mac,
            "cur_mac":  cur_mac,

            # MỚI: log cả hai kênh để bạn thấy ch1 có gì
            "prev_mac_rx0": self._extract_mac_fields(prev["meta_rx0"]),
            "prev_mac_rx1": self._extract_mac_fields(prev["meta_rx1"]),
            "cur_mac_rx0":  self._extract_mac_fields(cur["meta_rx0"]),
            "cur_mac_rx1":  self._extract_mac_fields(cur["meta_rx1"]),
        }

        self._write(rec)

    # giữ nguyên 2 hàm sau:
    def _extract_mac_fields(self, meta):
        def g(k, d=None): return self._get(meta, k, d)

        # freq_mhz: từ 'freq' (Hz) → MHz; fallback 'mac_freq_mhz'
        freq_mhz = None
        try:
            fhz = g("freq")
            if isinstance(fhz, (int, float)) and fhz > 0:
                freq_mhz = int(round(float(fhz)/1e6))
        except Exception:
            pass
        if freq_mhz is None:
            freq_mhz = g("mac_freq_mhz")

        # các trường MAC do estimator gắn
        seq   = g("seq",   g("mac_seq"))
        txid  = g("txid",  g("mac_txid"))
        txant = g("txant", g("mac_txant"))

        # thời điểm phát bên TX (nếu bạn đang truyền vào param ts)
        ts_air  = g("tx_unix_s", g("mac_ts_air"))

        # thời điểm RX thấy MAC (gắn ở estimator – xem phần B)
        t_rx_ms = g("mac_t_rx_ms", g("ts_wall_ms"))
    ...
    def _pick_mac(self, meta_a, meta_b):
        a = self._extract_mac_fields(meta_a); b = self._extract_mac_fields(meta_b)
        if a.get("seq") is not None: return a
        if b.get("seq") is not None: return b
        return a
...
class wifi_rx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        # --- Session UI: buộc nhập số trước khi Start ---
        self._session_toolbar = Qt.QToolBar(self)
        self._session_toolbar.addWidget(Qt.QLabel("Session ID:"))
        self._session_edit = Qt.QLineEdit()
        self._session_edit.setPlaceholderText("e.g., 001")
        self._session_edit.setValidator(QIntValidator(0, 10**9, self))  # chỉ cho số
        self._session_toolbar.addWidget(self._session_edit)

        self._session_path_label = Qt.QLabel("Will save under: (not set)")
        self._session_toolbar.addWidget(self._session_path_label)

        self.top_layout.addWidget(self._session_toolbar)

        # Trạng thái ban đầu: chưa hợp lệ thì không cho Start
        self._btn_fg_start.setEnabled(False)
        self._session_id = None
        self._session_root = None
        self._session_edit.textChanged.connect(self._on_session_text_changed)
        ...
        # Tap CSI để đưa vào logger
        self._cap0 = pdu_capture(lambda msg: self._on_csi_pdu(msg))
        self._cap1 = pdu_capture(lambda msg: self._on_csi_pdu(msg))
        self.msg_connect((self.csi_est0, 'csi'), (self._cap0, 'in'))
        self.msg_connect((self.csi_est1, 'csi'), (self._cap1, 'in'))
        ...
        # sau khi self.csi_est0/1 đã được tạo
        try:
            self.csi_est0._mac_window_ms = 400.0   # 300–500 ms tuỳ môi trường
            self.csi_est1._mac_window_ms = 400.0
        except Exception:
            pass
        ...
        self.COINC_WINDOW_MS = 5.0   # 2–5 ms là hợp lý cho LTF hai kênh
        # TTL cho coinc_id (nên >= 2 lần cửa sổ pair để cả 2 kênh kịp gắn)
        self.COINC_TTL_MS = max(2*self.COINC_WINDOW_MS, 600.0)
        ...
        # khoảng cách HAI anten PHÁT (đặt đúng theo bố trí thật)
        self.TX_ANT_SPACING_M = 1.60  # ví dụ 10 cm; bạn chỉnh theo thực tế

        # Logger AoD theo time-mux (TX0/TX1)
        self._txpairlogger = TxPairLogger(self.TX_ANT_SPACING_M, pair_window_ms= max(2*self.COINC_WINDOW_MS, 800.0))
 

    def _on_mac_pdu(self, freq_mhz:int, seq:int, txid:int, txant:int, ts:int):
        now_ms = time.monotonic()*1000.0
        self._last_mac = {'ts_ms': now_ms, 'freq_mhz': freq_mhz, 'seq': seq, 'txid': txid, 'txant': txant}
        try:
            self.csi_est0.set_last_mac(freq_mhz, seq, txid, now_ms, ts, txant)  # <<< THÊM txant
            self.csi_est1.set_last_mac(freq_mhz, seq, txid, now_ms, ts, txant)
        except Exception:
            pass
        print(f"[MAC] freq={freq_mhz} seq={seq} txid={txid} txant={txant} ts={ts}")
    ...
    def _switch_record_targets(self, freq_hz):
        ...
        pairs = os.path.join(base, f"aod_pairs_{ts}.jsonl")
        try:
            self._txpairlogger.set_output(pairs, freq_hz, self.TX_ANT_SPACING_M)
        except Exception:
            pass
    ...
    def _on_csi_pair_hit_slot(self):
        ...
        # >>> THÊM KHỐI NÀY Ở ĐÂY <<<
        coinc_token = int(time.monotonic()*1_000_000)        # token theo monotonic us
        ttl_ms = int(getattr(self, "COINC_TTL_MS", max(2*self.COINC_WINDOW_MS, 20.0)))
        try:
            self.csi_est0.set_pair_token(coinc_token, ttl_ms=ttl_ms)
            self.csi_est1.set_pair_token(coinc_token, ttl_ms=ttl_ms)
        except Exception:
            pass
        print(f"[PAIR] coinc_id={coinc_token} @ {int(self.freq/1e6)} MHz")
    ...
    def _fg_start_with_countdown(self, secs=3):
        if self._fg_is_running:
            return
        # NEW: bắt buộc chuẩn bị phiên (tạo Data/<SessionID>/ và đổi save_root)
    ...
    def _on_csi_pdu(self, msg):
        meta = pmt.car(msg)
        data = pmt.cdr(msg)
        try:
            self._txpairlogger.ingest(meta, data)
        except Exception:
            pass
    def _on_session_text_changed(self, s: str):
        """Bật Start khi Session ID là số hợp lệ; đồng thời hiển thị đường dẫn sẽ lưu."""
        if self._session_edit.hasAcceptableInput() and s.strip() != "":
            self._session_id = s.strip()
            # Gốc Data ban đầu:
            base_data = "/home/ea301b/gr-ieee802-11/examples/Data"
            # Thư mục phiên làm việc theo Session ID:
            self._session_root = os.path.join(base_data, self._session_id)
            self._session_path_label.setText(f"Will save under: {self._session_root}")
            self._btn_fg_start.setEnabled(True)
        else:
            self._session_id = None
            self._session_root = None
            self._session_path_label.setText("Will save under: (not set)")
            self._btn_fg_start.setEnabled(False)

    def _prepare_session(self) -> bool:
        """Khởi tạo thư mục phiên và chuyển đích save_root sang Data/<SessionID>/."""
        if not self._session_id:
            try:
                Qt.QMessageBox.warning(self, "Missing Session ID",
                                    "Bạn phải nhập Session ID (chỉ số) trước khi Start.")
            except Exception:
                print("Warning: Missing Session ID")
            return False
        try:
            os.makedirs(self._session_root, exist_ok=True)
        except Exception:
            pass
        # TỪ GIỜ: mọi file sẽ được ghi vào Data/<SessionID>/fXXXXMHz/...
        self.save_root = self._session_root
        self._session_edit.setEnabled(False)  # khóa ô nhập sau khi bắt đầu
        return True
