...
import ieee802_11
from csi_ltf_estimatorTest2 import csi_ltf_estimator
import json
...
class mac_freq_tap(gr.basic_block):
    ...
    def _handler(self, msg):
        try:
            ...
            # Thêm TXA=?
            # Bổ sung SWEEP và AOD trong regex (tùy chọn)
            m = re.search(
                br'FREQ=(\d{4});SEQ=(\d+);TS=(\d+)'
                br'(?:;TXA=(\d))?'
                br'(?:;PAIR=(-?\d+))?'
                br'(?:;PHI=(-?\d+))?'
                br'(?:;SWEEP=(\d+))?'     # NEW
            ...
            if m:
                ...
                seq      = int(m.group(2))
                txa  = int(m.group(4)) if (m.lastindex and m.group(4) is not None) else None
                pair = int(m.group(5)) if (m.lastindex and m.group(5) is not None) else None
                phi  = int(m.group(6)) if (m.lastindex and m.group(6) is not None) else None
                sweep= int(m.group(7)) if (m.lastindex and m.group(7) is not None) else None   # NEW
                aod  = int(m.group(8)) if (m.lastindex and m.group(8) is not None) else None   # NEW
                self.cb(freq_mhz, seq, txa, pair, phi, sweep, aod)   # CHANGED chữ ký callback
...
class wifi_rx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        sym_samps = int(round(80 * (20e6 / float(self.samp_rate))))
        # 5 MHz → 320, 10 MHz → 160, 20 MHz → 80
        self.csi_est0.set_timebase(self.samp_rate, sym_samps)
        self.csi_est1.set_timebase(self.samp_rate, sym_samps)

        # NEW: nới cửa sổ ghép MAC–CSI để 'seq' xuất hiện ổn định hơn
        self.csi_est0.set_mac_window_ms(500.0)
        self.csi_est1.set_mac_window_ms(500.0)
        ...
        # --- SESSION ID (folder 1..100) ---
        self.run_id = None                       # NEW: mã phiên người dùng nhập
        self.session_dir = self.save_root        # NEW: gốc ghi mặc định (chưa set thì = save_root)

        # NEW: Toolbar nhập Session ID
        self._session_toolbar = Qt.QToolBar(self)
        self._session_toolbar.addWidget(Qt.QLabel("Session ID (1–100): "))

        self._session_spin = Qt.QSpinBox()
        self._session_spin.setRange(1, 100)
        self._session_spin.setValue(1)
        self._session_toolbar.addWidget(self._session_spin)

        self._btn_set_session = Qt.QPushButton("Set Session")
        self._session_toolbar.addWidget(self._btn_set_session)

        self._session_label = Qt.QLabel("Folder: (not set)")
        self._session_toolbar.addSeparator()
        self._session_toolbar.addWidget(self._session_label)

        self.top_layout.addWidget(self._session_toolbar)

        def _update_session_label():
            path = os.path.join(self.save_root, str(self.run_id)) if self.run_id is not None else "(not set)"
            self._session_label.setText(f"Folder: {path}")

        def _set_session_clicked():
            try:
                rid = int(self._session_spin.value())
                self.run_id = rid
                self.session_dir = os.path.join(self.save_root, str(rid))
                os.makedirs(self.session_dir, exist_ok=True)
                _update_session_label()
                print(f"[SESSION] set to {self.session_dir}")
            except Exception as e:
                print("[SESSION] set failed:", e)

        self._btn_set_session.clicked.connect(_set_session_clicked)
        _update_session_label()
        
        # --- Move Session toolbar above the Start toolbar ---
        try:
            idx_fg = self.top_layout.indexOf(self._fg_toolbar)       # CHỈNH THỨ TỰ HIỂN THỊ
            if idx_fg != -1:
                self.top_layout.removeWidget(self._session_toolbar)   # gỡ khỏi vị trí hiện tại
                self.top_layout.insertWidget(idx_fg, self._session_toolbar)  # chèn lên trên FG toolbar
        except Exception:
            pass
        ...
        # self.SEQ_FREQS      = [5180e6]

        # self.N_REQUIRED     = 5          # cần bắt đủ 5 lần mỗi tần số
        self.SEQ_TIMEOUT_MS = 20000      # tối đa 15s mỗi tần số rồi nhảy tiếp
        self._seq_idx       = 0
        self._seq_hits      = 0

        # --- dwell / idle tracking (PHẢI có trước khi _start_seq_round) ---
        self.MIN_DWELL_MS = 20000   # tối thiểu ở lại 12s
        self.MAX_DWELL_MS = 20000   # tối đa 18s (watchdog)
        ...
        self.COINC_WINDOW_MS = 3.0
        # YÊU CẦU số kênh tối thiểu cùng “hit” để mở ghi (giảm từ 3 xuống 2)
        self.MIN_CHANNELS_FOR_HIT = 2
        self._last_csi_ms = [-1.0, -1.0, -1.0, -1.0]  # 4 kênh
        self._seq_active = False
        # trong __init__
    ...
    def _on_mac_pdu(self, freq_mhz:int, seq:int, txa:int|None, pair:int|None, phi:int|None,
                    sweep:int|None, aod:int|None):   # NEW
        now_ms = time.monotonic()*1000.0
        self._last_mac = {
            'ts_ms': now_ms, 'freq_mhz': freq_mhz, 'seq': seq,
            'txa': txa, 'pair': pair, 'phi': phi,
            'sweep_id': sweep, 'aod_idx': aod          # NEW
        }

        # đẩy xuống estimator cả 2 kênh
        try:
            self.csi_est0.set_last_mac(freq_mhz, seq, ts_ms=now_ms,
                                    txa=txa, pair_id=pair, phi_deg=phi,
                                    sweep_id=sweep, aod_idx=aod)   # NEW
            self.csi_est1.set_last_mac(freq_mhz, seq, ts_ms=now_ms,
                                    txa=txa, pair_id=pair, phi_deg=phi,
                                    sweep_id=sweep, aod_idx=aod)   # NEW
        except Exception:
            pass

        print(f"[MAC] f={freq_mhz} seq={seq} txa={txa} pair={pair} phi={phi} sweep={sweep} aod={aod} (ts={now_ms:.1f})")

        # Ghi vào MAC JSONL (nếu đang mở)
        try:
            if getattr(self, "mac_fh", None):
                rec = {
                    "ts_wall_ms": int(time.time()*1000.0),
                    "freq_rx_mhz": int(round(self.freq/1e6)),
                    "freq_tx_mhz": int(freq_mhz),
                    "seq": int(seq),
                    "txa": (int(txa) if txa is not None else None),
                    "pair_id": (int(pair) if pair is not None else None),
                    "phi_deg": (int(phi) if phi is not None else None),
                    "sweep_id": (int(sweep) if sweep is not None else None),   # NEW
                    "aod_idx": (int(aod) if aod is not None else None)         # NEW
                }
                self.mac_fh.write(json.dumps(rec, ensure_ascii=False) + "\n")
        except Exception:
            pass
    ...
    def _switch_record_targets(self, freq_hz):
        ...
        base  = os.path.join(self.session_dir, f"f{f_mhz}MHz")          # NEW: ghi vào folder phiên
        self._safe_makedirs(base)
        ...
        macj  = os.path.join(base, f"mac_{ts}.jsonl")     # NEW
        ...
        try:
            ...
            pass
        try:
            self.csi_est0.set_meta_every_n(1)
            self.csi_est1.set_meta_every_n(1)
        except Exception:
            pass
        # NEW: mở log MAC
        try:
            if getattr(self, "mac_fh", None):
                try: self.mac_fh.close()
                except Exception: pass
            self.mac_fh = open(macj, "a", buffering=1, encoding="utf-8")
        except Exception:
            self.mac_fh = None

        print(f"[REC] CH0 → {bin0}")
        print(f"[REC] CH1 → {bin1}")
        print(f"[REC] MAC → {macj}")     # NEW
    ...
    ################################################################################
    def _fg_start(self):
        if not self._require_session():    # NEW
            return        
        try:
            self.start()
    ...
    def _on_csi_pair_hit_slot(self):
        ...
        # Gate theo MAC: phải tươi, đúng FREQ và TXA=2
        last = getattr(self, "_last_mac", {}) or {}
        f_rx  = int(round(self.freq / 1e6))
        fresh = (last.get('ts_ms', -1) > 0) and ((now_ms - last['ts_ms']) <= self.MAC_MATCH_WINDOW_MS)
        ok    = fresh and (last.get('freq_mhz') == f_rx) and (last.get('txa') == 2)
        if self.STRICT_FREQ_GATE:
            if not ok:
                return
        else:
            # Dù không strict vẫn yêu cầu TXA=2 nếu có MAC
            if last.get('txa') is not None and last.get('txa') != 2:
                return
    ...
    def _require_session(self):
        if self.run_id is None:
            try:
                Qt.QMessageBox.warning(self, "Session required",
                    "Hãy nhập một số 1–100 và bấm 'Set Session' trước khi Start.")
            except Exception:
                print("[WARN] Session ID is required before starting flowgraph.")
            return False
        return True
