...
class wifi_tx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.pdu_length = pdu_length = 1200
        ...
        self.blocks_message_strobe_0_0 = blocks.message_strobe(self._make_pdu(self.pdu_length), self.interval)
        self.set_pdu_length(self.pdu_length)
        self.set_interval(self.interval)
                
        # self.blocks_message_debug_0 = blocks.message_debug()


        # TX spectrum/time just before USRP
        self.qtgui_freq_sink_x_0 = qtgui.freq_sink_c(
            1024, window.WIN_BLACKMAN_hARRIS, self.freq, self.samp_rate,
            "TX Spectrum (baseband before USRP)", 1)
        self._qtgui_freq_sink_x_0_win = sip.wrapinstance(
            self.qtgui_freq_sink_x_0.qwidget(), Qt.QWidget)
        self.top_layout.addWidget(self._qtgui_freq_sink_x_0_win)

        self.qtgui_time_sink_x_0 = qtgui.time_sink_c(
            1024, self.samp_rate, "TX IQ (baseband before USRP)", 1)
        ...
        # Connections
        ##################################################
        # self.dbg_mac_out = blocks.message_debug()
        ...
        # ---- BYPASS MAC: Strobe -> PHY ----
        # self.msg_connect((self.blocks_message_strobe_0_0, 'strobe'), (self.wifi_phy_hier_0, 'mac_in'))
        
        # self.dbg_mac_out = blocks.message_debug()
        # self.msg_connect((self.ieee802_11_mac_0, 'phy out'), (self.dbg_mac_out, 'print'))
        # self.dbg_phy_in = blocks.message_debug()
        self.msg_connect((self.ieee802_11_mac_0, 'phy out'), (self.wifi_phy_hier_0, 'mac_in'))
        # self.msg_connect((self.ieee802_11_mac_0, 'phy out'), (self.dbg_phy_in, 'print_pdu'))
        ...
        # ---- PROBE RATE (in Msps) ----
        self.blocks_probe_rate_0 = blocks.probe_rate(gr.sizeof_gr_complex, 500.0, 0.01)
        self.connect((self.wifi_phy_hier_0, 0), (self.blocks_probe_rate_0, 0))
        def print_rate():
            print("[TX rate] %.2f Msps" % (self.blocks_probe_rate_0.rate()/1e6))
        self.timer_pr = Qt.QTimer(self)     # giữ reference
        self.timer_pr.timeout.connect(print_rate)
        self.timer_pr.start(1000)
        ...
        # đã có: wifi_phy_hier -> multiply_const -> packet_pad2 -> USRP
    ...
    def _make_pdu(self, nbytes=500, fill=0x42):
        payload = pmt.make_u8vector(nbytes, fill)   # mảng uint8
        return pmt.cons(pmt.PMT_NIL, payload)       # (meta, payload)
    ...
    def set_pdu_length(self, pdu_length):
        ...
        self.blocks_message_strobe_0_0.set_msg(self._make_pdu(self.pdu_length))
