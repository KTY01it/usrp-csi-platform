...
class wifi_tx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        # ====== CẤU HÌNH KÊNH TX ======
        # Nếu bạn CHẮC CHẮN có 2 TX (ví dụ 2 mboard hoặc dual-TX subdev), đổi USE_DUAL_TX=True và thêm addr1=...
        self.USE_DUAL_TX = True
        tx_channels = [0,1]

        # devargs = "addr=192.168.10.2"  # THƯỜNG DÙNG VỚI N210; nếu auto-discovery, có thể để rỗng ""
        ...
        # Áp dụng tần số & gain cho MỌI kênh khả dụng
        for ch in tx_channels:
            ...
            self.uhd_usrp_sink_0.set_normalized_gain(tx_gain, ch)
        ...
        self.tx0_gate = blocks.multiply_const_cc(1.0)  # TX0 bật lúc khởi động
        self.tx1_gate = blocks.multiply_const_cc(0.0)  # TX1 tắt lúc khởi động
        ...
        # Kết nối tới các kênh TX hiện có
        ...
        # THAY BẰNG:
        self.connect((self.foo_packet_pad2_0, 0), (self.tx0_gate, 0))
        self.connect((self.tx0_gate, 0), (self.uhd_usrp_sink_0, 0))
        self.connect((self.foo_packet_pad2_0, 0), (self.tx1_gate, 0))
        self.connect((self.tx1_gate, 0), (self.uhd_usrp_sink_0, 1))
        
        # --- HOP cấu hình ---
        self.HOP_ENABLE   = True
        self.HOP_RANDOM   = False           # True: ngẫu nhiên, False: round-robin
        self.HOP_INTERVAL = 15.0            # giây, tuỳ bạn
        self.HOP_FREQS    = [5180e6, 5200e6, 5220e6, 5240e6, 5260e6, 5280e6, 5300e6, 5320e6]
        self._hop_idx     = -1
        self._hop_seq     = 0

        # payload mặc định lần đầu (để chắc chắn có FREQ ngay cả trước khi hop)
        ...
        # --- TX ID ổn định (để đối chiếu ở RX) ---
        self.TX_ID = 1  # hoặc random.randint(1, 9999) nếu muốn


        # --- Time-multiplexed sounding ---
        self.SOUND_ENABLE       = True
        self.SOUND_INTERVAL_MS  = int(self.interval)  # mỗi khoảng strobe (vd 100ms) đổi TX
        self._tx_active         = 0  # 0 -> TX0, 1 -> TX1

        self._sounding_timer = QtCore.QTimer(self)
        self._sounding_timer.setInterval(self.SOUND_INTERVAL_MS)
        self._sounding_timer.timeout.connect(self._toggle_txant)
    ...
    def set_tx_gain(self, tx_gain):
        ...
        # trước đây: chỉ ch=0; nếu dual: lặp
        if self.USE_DUAL_TX:
            for ch in [0,1]:
                self.uhd_usrp_sink_0.set_normalized_gain(self.tx_gain, ch)
        else:
    ...
    def set_interval(self, interval):
        ...
        # cập nhật timer sounding để luôn trùng với nhịp frame
        self.SOUND_INTERVAL_MS = int(self.interval)
        try:
            self._sounding_timer.setInterval(self.SOUND_INTERVAL_MS)
        except Exception:
            pass
    ...
    #         return
    #     try:
    #         self.start()  # chạy flowgraph
    #         self._fg_running = True
    #         self._tx_status.setText("TX: running")
    #         print("[TX] START")

    #         # Bắt đầu hop & dwell counter khi TX chạy
    #         if self.HOP_ENABLE:
    #             self._hop_timer.start()
    #         self._dwell_count_sec = 0
    #         self._dwell_count_timer.start()
    ...
    def _start_tx(self):
        ...
        try:
            ...
            self._do_hop()             # hop tức thì về 5180 và cập nhật payload/seq
            if self.HOP_ENABLE:
                self._hop_timer.start()

            self._dwell_count_sec = 0
            self._dwell_count_timer.start()   # chỉ bắt đầu đếm sau khi đã hop
        except Exception as e:
            print("[TX] START failed:", e)
    ...
    def _stop_tx(self):
        ...
        try:
            ...
            try: self._sounding_timer.stop()
            except Exception: pass
            # Tắt cả hai kênh khi dừng
            self.tx0_gate.set_k(0.0); self.tx1_gate.set_k(0.0)
    ...
    def _update_strobe_payload(self, freq_hz: float, txid: int=None, txant: int=None):
        try:
            self._hop_seq += 1
            freq_mhz = int(round(freq_hz / 1e6))
            if txid  is None: txid  = self.TX_ID
            if txant is None: txant = self._tx_active
            s = f"FREQ={freq_mhz};SEQ={self._hop_seq};TXID={txid};TXANT={int(txant)};TS={int(time.time())}"
            b = s.encode("ascii")
            vec = pmt.init_u8vector(len(b), list(b))
            msg = pmt.cons(pmt.PMT_NIL, vec)
            self.blocks_message_strobe_0_0.set_msg(msg)
            print(f"[TX{txid}] seq={self._hop_seq} freq={freq_mhz}MHz TXANT={int(txant)} ts={int(time.time())}")
        except Exception as e:
    ...
    def _do_hop(self):
        ...
        self.set_freq(f)
        # cập nhật payload để RX đọc được FREQ/SEQ/TXID/TS
        self._update_strobe_payload(f, self.TX_ID, self._tx_active)
    ...
    def _set_active_txant(self, txant: int):
        txant = 0 if int(txant) == 0 else 1
        if txant == 0:
            self.tx0_gate.set_k(1.0); self.tx1_gate.set_k(0.0)
        else:
            self.tx0_gate.set_k(0.0); self.tx1_gate.set_k(1.0)
        self._tx_active = txant
        # Cập nhật payload để RX biết TXANT hiện tại
        try:
            self._update_strobe_payload(self.freq, self.TX_ID, self._tx_active)
        except Exception:
            pass
        print(f"[TX] SOUND → TXANT={self._tx_active}")

    def _toggle_txant(self):
        self._set_active_txant(1 - self._tx_active)
